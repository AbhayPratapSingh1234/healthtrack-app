...
serve(async (req) => {
  ...
  try {
    ...
    console.log("📥 Payload:", Object.keys(assessmentData));
    
    if (!assessmentData || typeof assessmentData !== 'object') {
      return new Response(
        JSON.stringify({ error: "Missing assessmentData" }),
...
List complications (heart attack, stroke, kidney failure etc) for each disease.`;
    ...
    if (!response.ok) {
      return new Response(
        JSON.stringify({ error: `API ${response.status}`, debug: text }),
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ error: "JSON parse failed", debug: text.substring(0, 500) }),
